...
export default function TelaAtividade() {
  ...
  async function handleSalvar() {
    ...
    if (isEditar && atividade) {
      await atualizarAtividade(atividade.id, {
        ...
        tecnica: temTimer ? (atividade.tecnica ?? 'foco') : undefined,
        ehModelo,
      ...
      const novaAtividade: Atividade = {
        ...
        temTimer,
        duracao: temTimer ? duracao : undefined,
        tecnica: temTimer ? 'foco' : undefined,
        concluida: false,
        data: data as string,
        ordem: Number(ordem),
        ehModelo,
  ...
  return (
    <ScrollView style={styles.criarContainer}>
      ...
      <TextInput style={styles.inputTexto} value={titulo} onChangeText={setTitulo} placeholder="Ex: Estudar React Native" placeholderTextColor={theme.textDimmer} />

      <Text style={styles.label}>Horário (opcional)</Text>
      <View style={styles.inputRow}>
        <TextInput style={styles.inputNumero} value={horaH} onChangeText={setHoraH} keyboardType="number-pad" maxLength={2} placeholder="09" placeholderTextColor={theme.textDimmer} />
        <Text style={styles.inputSep}>:</Text>
        <TextInput style={styles.inputNumero} value={horaM} onChangeText={setHoraM} keyboardType="number-pad" maxLength={2} placeholder="00" placeholderTextColor={theme.textDimmer} />
      </View>

      <Text style={styles.label}>Descrição (opcional)</Text>
      <TextInput style={[styles.inputTexto, styles.inputArea]} value={descricao} onChangeText={setDescricao} placeholder="Detalhes da atividade..." placeholderTextColor={theme.textDimmer} multiline />
      ...
      <View style={styles.toggleRow}>
        ...
        <Switch
          value={temTimer}
          onValueChange={setTemTimer}
          trackColor={{ false: theme.switchTrackOff, true: theme.switchTrackOff }}
          thumbColor={temTimer ? theme.btnPrimaryBg : theme.switchThumbInactive}
        />
      </View>
      ...
      {temTimer && (
        <>
          ...
          <View style={styles.inputRow}>
            <TextInput style={styles.inputNumero} value={minutosFoco} onChangeText={setMinutosFoco} keyboardType="number-pad" maxLength={2} placeholder="25" placeholderTextColor={theme.textDimmer} />
            <Text style={styles.inputSep}>m</Text>
            <TextInput style={styles.inputNumero} value={segundosFoco} onChangeText={setSegundosFoco} keyboardType="number-pad" maxLength={2} placeholder="0" placeholderTextColor={theme.textDimmer} />
      ...
      <View style={styles.toggleRow}>
        ...
        <Switch
          value={ehModelo}
          onValueChange={setEhModelo}
          trackColor={{ false: theme.switchTrackOff, true: theme.switchTrackOff }}
          thumbColor={ehModelo ? theme.btnPrimaryBg : theme.switchThumbInactive}
        />
      </View>
      ...
      <View style={styles.novaEtapaContainer}>
        <TextInput style={styles.inputTexto} value={novaEtapaTitulo} onChangeText={setNovaEtapaTitulo} placeholder="Título da etapa..." placeholderTextColor={theme.textDimmer} />
        <View style={styles.toggleRow}>
          <View>
          ...
          <Switch
            value={novaEtapaTemTimer}
            onValueChange={setNovaEtapaTemTimer}
            trackColor={{ false: theme.switchTrackOff, true: theme.switchTrackOff }}
            thumbColor={novaEtapaTemTimer ? theme.btnPrimaryBg : theme.switchThumbInactive}
          />
        </View>
        {novaEtapaTemTimer && (
          <View style={styles.inputRow}>
            <TextInput style={styles.inputNumero} value={novaEtapaMinutos} onChangeText={setNovaEtapaMinutos} keyboardType="number-pad" maxLength={2} placeholder="5" placeholderTextColor={theme.textDimmer} />
            <Text style={styles.inputSep}>m</Text>
            <TextInput style={styles.inputNumero} value={novaEtapaSegundos} onChangeText={setNovaEtapaSegundos} keyboardType="number-pad" maxLength={2} placeholder="0" placeholderTextColor={theme.textDimmer} />
            <Text style={styles.inputSep}>s</Text>
          </View>
        )}
...
function makeStyles(t: Theme) {
  return StyleSheet.create({
    execContainer:          { flex: 1, backgroundColor: t.bg, alignItems: 'center', justifyContent: 'center', padding: 24 },
    execTitulo:             { fontSize: 28, fontWeight: '300', color: t.textPrimary, textAlign: 'center', marginBottom: 8 },
    execDesc:               { fontSize: 14, color: t.textMuted, textAlign: 'center', marginBottom: 24 },
    execMeta:               { fontSize: 13, color: t.textFaint, marginBottom: 8 },
    ...
    criarContainer:         { flex: 1, backgroundColor: t.bg },
    criarTitulo:            { fontSize: 28, fontWeight: '300', color: t.textPrimary, marginBottom: 32, paddingHorizontal: 24 },
    label:                  { fontSize: 11, color: t.textMuted, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, marginTop: 20, paddingHorizontal: 24 },
    inputTexto:             { backgroundColor: t.card, borderWidth: 1, borderColor: t.border, borderRadius: 12, padding: 14, color: t.textPrimary, fontSize: 15, marginHorizontal: 24 },
    inputArea:              { height: 80, textAlignVertical: 'top' },
    toggleRow:              { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, backgroundColor: t.card, borderRadius: 12, borderWidth: 1, borderColor: t.border, margin: 24 },
    ...
    inputNumero:            { width: 56, height: 48, backgroundColor: t.card, borderRadius: 8, borderWidth: 1, borderColor: t.border, color: t.textPrimary, fontSize: 20, textAlign: 'center' },
    inputSep:               { color: t.textDimmer, fontSize: 18 },
    etapaItem:              { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 12, backgroundColor: t.card, borderRadius: 10, marginHorizontal: 24, marginBottom: 6, borderWidth: 1, borderColor: t.border },
    etapaInfo:              { flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 },
    etapaNumero:            { fontSize: 12, color: t.textFaint, width: 16 },
    etapaTitulo:            { fontSize: 14, color: t.textPrimary, marginBottom: 2 },
    etapaDuracao:           { fontSize: 11, color: t.textMuted },
    etapaRemover:           { color: t.textDimmer, fontSize: 16, paddingHorizontal: 8 },
    novaEtapaContainer:     { gap: 0 },
    btnAdicionarEtapa:      { padding: 12, borderRadius: 10, borderWidth: 1, borderColor: t.borderAccent, alignItems: 'center', marginHorizontal: 24, marginTop: 4 },
    btnAdicionarEtapaTexto: { color: t.textMuted, fontSize: 13 },
